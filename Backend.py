...
conn.commit()
cursor.execute('CREATE TABLE IF NOT EXISTS Notes(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER ,content TEXT)')
conn.commit()
...
def add_notes(user_id,content):
  ...
  notes_collection.add(
      documents = [content],
      ids = [str(note_id)], metadatas = [{'user_id':user_id}]
  )
def get_notes(user_id):
  cursor.execute('SELECT * FROM Notes WHERE user_id = ?',(user_id,))
  return cursor.fetchall()
def search_notes(user_id,question):
  results = notes_collection.query(query_texts = [question],where= {'user_id': user_id} ,n_results = 2)
...
def ask_ai(user_id,question):
    ...
    memory_collection.add(
                ...
                ids = [str(cursor.lastrowid)], metadatas=[{"user_id":user_id}]
    )
    return answer
def search_memory(user_id,question):
      results = memory_collection.query(query_texts = [question],where= {'user_id': user_id} ,n_results = 2)
      return results['documents'][0]
...
def create_user(item:USERINPUT):
  try:
    ...
    print(item)
    add_users(item.username,item.email,item.password)
...
def login(item:LOGINUSER):
  ...
  if user:
    return{ 'message': 'logined susccessfully',
           'user_id' : user[0]}
  else:
      raise HTTPException(
          status_code = 404,
...
def delete_notes(delete_id:int):
  cursor.execute('Delete FROM Notes WHERE id = ?',(delete_id,))#rememeber that we use i in place of user_id cause id increases but with use_id =1 we can have thousands of users so all will be deleted as id keep increasing but user_id can be shared by many users
  conn.commit()
